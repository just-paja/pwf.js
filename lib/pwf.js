var pwf = function()
{
	...
	/// Extend and clone all properties of object
		...
		Object.merge = function() {
			...
			for (var i = 0; i < args.length; i++) {
				...
				for (var property in source) {
					if (deep && Object.prototype.toString.call(source[property]) == '[object Object]') {
						destination[property] = arguments.callee(deep, destination[property], source[property]);
					} else if (deep && Object.prototype.toString.call(source[property]) == '[object Array]') {
	...
	return new function()
	{
		...
		this.wrap_public_method = function(proto, cont, key)
		{
			...
			/** Check if this method has been wrapped already. Wrap the inner one
			 * if so
			 */
		 ...
		 * @return this
		 */
		this.wrap_static_method = function(cdef, key)
		{
			...
			if (typeof opts.fn == 'function') {
				...
				for (var key in opts.static) {
					if (typeof opts.static[key] == 'function') {
						this.wrap_static_method(opts, key);
					}
				}
		...
		this.extend_class = function(class_opts)
		{
			...
			(typeof class_opts.static == 'undefined') && (class_opts.static = {});
			(typeof class_opts.public == 'undefined') && (class_opts.public = {});
			(typeof class_opts.proto  == 'undefined') && (class_opts.proto  = {});
			...
			if (parents_ok) {
				...
				for (var iparent = 0, plength = class_opts.parents.length; iparent < plength; iparent++) {
					...
					if (typeof parent_opts.storage == 'object') {
						storage = Object.merge(true, storage, parent_opts.storage);
					}

					static = Object.merge(static, parent_opts.static);
					public = Object.merge(public, parent_opts.public);
					proto  = Object.merge(proto, parent_opts.proto);
					meta   = Object.merge(true, meta, parent_opts.meta);

					for (var i = 0; i < parent_opts.fn.parents.length; i++) {
						parents.push(parent_opts.fn.parents[i]);
					}

					parents.push(parent_opts.name);

					if (typeof parent_opts.fn.init == 'object') {
						for (var iinit = 0; iinit < parent_opts.fn.init.length; iinit++) {
							init.push(parent_opts.fn.init[iinit]);
						}
					}
				}

				if (typeof class_opts.init != 'undefined') {
					init.push(class_opts.init);
				}

				if (typeof class_opts.fn == 'undefined') {
					class_opts.fn = function() {};
				}

				meta                  = Object.merge(true, meta, class_opts.meta);
				class_opts.static     = Object.merge(static, class_opts.static);
				class_opts.public     = Object.merge(public, class_opts.public);
				class_opts.proto      = Object.merge(proto,  class_opts.proto);
				class_opts.storage    = Object.merge(true, storage, class_opts.storage);
				class_opts.fn.parents = parents;
				class_opts.fn.init    = init;
			}


			if (typeof class_opts.fn.init == 'object' && class_opts.fn.init.length) {
				/** Assemble prototype init function from all inherited inits of parent
				 *  classes
				 */
				class_opts.public.init = function(list) {
					return function() {
			...
			class_opts.fn.prototype.meta = meta;

			meta.constructor = class_opts.fn;
			meta.cname       = class_opts.name;
			meta.static      = class_opts.static;
			meta.parents     = class_opts.fn.parents;
		...
		this.extend_context = function(obj, storage_proto, args)
		{
			...
			if (typeof storage_proto != 'object' || storage_proto === null) {
				storage_proto = {};
			}

			storage = Object.merge(true, storage_proto);

			for (var key in obj) {
				if (typeof obj[key] == 'function') {
					var method = obj[key];

					if (typeof method.sp == 'function') {
						method = method.sp;
					}

					obj[key] = function(obj, method, storage) {
						return function() {
							var
								args = Array.prototype.slice.call(arguments),
								retval;
		...
		this.create = function()
		{
			var
				...
				obj = this.extend_context(new constructor(args), class_def.storage, args);
		...
		/** List classes in a scope
		 * @param string scope
		 * @return list
		 */
		this.list_scope = function(scope)
		{
			var list = [];

			for (var cname in classes) {
				if (cname.indexOf(scope) === 0) {
					list.push(cname);
				}
			}

	...
	var Pwf = function()
	{
		...
		Internal.prototype.extend_class = function(opts)
		{
			var
				...
				merge_list = Object.keys(merge),
				merge_size = merge_list.length;
			...
			if (opts.parents.length) {
				for (var ipar = 0, plen = opts.parents.length; ipar < plen; ipar++) {
					...
					for (var imer = 0; imer < merge_size; imer++) {
						mk = merge_list[imer];
						merge[mk].push(p_opts[mk]);
					}
				...
				for (var imer = 0; imer < merge_size; imer++) {
					mk = merge_list[imer];
					merge[mk].push(opts[mk]);
					opts[mk] = self.merge.apply(self, merge[mk]);
				}
			}

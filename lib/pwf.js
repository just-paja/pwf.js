				...
				if (parents_ok) {
					...
					for (var iparent = 0, plength = class_opts.parents.length; iparent < plength; iparent++) {
						...
						if (typeof parent_opts.storage == 'object') {
							storage = this.merge(true, storage, parent_opts.storage);
						}

						static = this.merge(static, parent_opts.static);
						public = this.merge(public, parent_opts.public);
						proto  = this.merge(proto, parent_opts.proto);
						meta   = this.merge(true, meta, parent_opts.meta);
					...
					meta                  = this.merge(true, meta, class_opts.meta);
					class_opts.static     = this.merge(static, class_opts.static);
					class_opts.public     = this.merge(public, class_opts.public);
					class_opts.proto      = this.merge(proto,  class_opts.proto);
					class_opts.storage    = this.merge(true, storage, class_opts.storage);

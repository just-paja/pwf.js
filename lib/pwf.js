var pwf = function()
{
	...
	return new function()
	{
		...
		this.extend_proto = function(name, opts, list_parents)
		{
			var
				proto = typeof opts[name] == 'object' ? opts[name]:{},
				proto_ok = proto !== null && typeof proto == 'object',
				parents_ok = list_parents !== null && typeof list_parents == 'object' && typeof list_parents.length == 'number';

			if (proto_ok && parents_ok) {
				var obj = {};

				for (var iparent = 0, plength = list_parents.length; iparent < plength; iparent++) {
					//~ this.extend_obj(proto, )
					v(['extending', proto_name, list_parents[iparent]]);
				}

				protos[proto_name] = this.extend_obj(obj, protos[proto_name]);
				protos[proto_name].parents = list_parents;
			}
		...
		this.register_class = function(opts)
		{
			if (typeof opts.name == 'string') {
				var con_opts = {
					'public':this.extend_obj('public', opts, opts.parents),
					'proto':this.extend_obj('proto', opts, opts.parents),
				};

				classes[opts.name] = {
					'constructor':this.extend_constructor(opts),
					'opts':opts
				};

			} else throw new Error('pwf-register-class-invalid-args');

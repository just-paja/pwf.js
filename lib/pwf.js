(function() {
	...
	var pwf = function()
	{
		return new function()
		{
			 ...
			 */
			this.extend_class = function(copts)
			{
				var
					meta = {},
					parents_ok =
						typeof copts.parents == 'object' &&
						copts.parents !== null &&
						typeof copts.parents.length == 'number';

				(!('storage' in copts)) && (copts.storage = {});
				(!('static'  in copts)) && (copts.static  = {});
				(!('public'  in copts)) && (copts.public  = {});
				(!('proto'   in copts)) && (copts.proto   = {});
				(!('fn'      in copts)) && (copts.fn = function() {});
				...
				if (parents_ok) {
					...
					for (var iparent = 0, plength = copts.parents.length; iparent < plength; iparent++) {
						var
							parent_name = copts.parents[iparent],
					...
					if (copts.init != null) {
						if (init.indexOf(copts.init) < 0) {
							init.push(copts.init);
						}
					}

					meta             = this.merge(true, meta, copts.meta);
					copts.static     = this.merge(static, copts.static);
					copts.public     = this.merge(public, copts.public);
					copts.proto      = this.merge(proto,  copts.proto);
					copts.storage    = this.merge(true, storage, copts.storage);
					copts.fn.parents = parents;
					copts.fn.init    = init;
				}


				if (typeof copts.fn.init == 'object' && copts.fn.init.length) {
					/** Assemble prototype init function from all inherited inits of parent
					 *  classes
					 */
					copts.public.init = function(list) {
						return function() {
				...
				copts.fn = this.extend_constructor(copts);
				copts.fn.prototype.meta = meta;

				meta.constructor = copts.fn;
				meta.cname       = copts.name;
				meta.static      = copts.static;
				meta.parents     = copts.fn.parents;

(function() {
	...
	var pwf = function()
	{
		return new function()
		{
			...
			var nomerge_types = ['boolean', 'string', 'number'];
			...
			this.extend_class = function(class_opts)
			{
				...
				(typeof class_opts.storage == 'undefined') && (class_opts.storage = {});
				...
				if (parents_ok) {
					...
					for (var iparent = 0, plength = class_opts.parents.length; iparent < plength; iparent++) {
						...
						if (parent_opts.fn.init !== null) {
							for (var iinit = 0; iinit < parent_opts.fn.init.length; iinit++) {
								if (init.indexOf(parent_opts.fn.init[iinit]) < 0) {
					...
					if (class_opts.init != null) {
						if (init.indexOf(class_opts.init) < 0) {
			...
			{
				return name in classes ? classes[name]:null;
			};

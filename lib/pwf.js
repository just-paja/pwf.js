		...
		return new function()
		{
			...
			var wrap_static_method = function(cdef, key)
			{
				...
				cdef[key] = function(cdef, key, method) {
					return function() {
						return method.apply(cdef, Array.prototype.slice.call(arguments));
					};
				}(cdef, key, method);
			};
			...
			var save_class = function(ctrl, opts)
			{
				if (opts instanceof Object && typeof opts.name == 'string') {
					...
					if (opts.config instanceof Function) {
						opts.config();
					}
			...
			this.obj_search = function(source, name)
			{
				...
				if (ref instanceof Object && typeof name == 'string' && name.indexOf('.') >= 0) {
					var path = name.split('.');

					while (path.length > 1) {
						seg = path.shift();

						if (ref[seg] instanceof Object) {
							ref = ref[seg];
						} else {
							ref = null;
							break;
				...
				return (!(ref instanceof Object) || typeof ref[name] == 'undefined') ? null:ref[name];
			};

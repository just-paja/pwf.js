(function() {
	...
	var pwf = function()
	{
		return new function()
		{
			...
			{
				return module in self && (!(self[module].is_ready instanceof Function) || self[module].is_ready());
			};
			...
			internal.prototype.init = function(module)
			{
				if (typeof self[module].init == 'function') {
					if (status.module[module] != internal.states.initialized) {
						var stat = self[module].init(function(pwf, status) {
			...
			this.reg_class = function(name, opts)
			{
				...
				if (typeof opts.name == 'string') {
					...
					} else throw new Error(['pwf:reg_class:already_exists', opts.name]);
			...
			this.status = function(name) {
				if (typeof status.module[name] != 'undefined') {
					if (status.module[name] == internal.states.initialized) {
						return true;
					}
